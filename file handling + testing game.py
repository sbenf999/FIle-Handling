...
def test_knowledge(filename):
    ...
    while questions_asked != len(test_file):
        def_or_keyword = random.choice([0,1])
        ...
        if def_or_keyword == 0:
            ...
            print(f"\nQuestion {question_number}) The random keyword is '{test_file[rand_index][0]}'. What is the corresponding defintion?")
            printList(choices)
            ...
            for i in range(4):
                ...
                choices.append([(test_file[random_][0]).strip("\n"), random_])
            ...
            chosen_ = ((test_file[rand_index][1])[1:]).strip("\n")
            
            print(f"\nQuestion {question_number}) The random definition is '{chosen_}'. What is the corresponding keyword?")
            printList(choices)
            choice = get_input("> ", [0, 1, 2, 3])

            if (test_file[rand_index][0]).strip("\n") == (choices[choice][0]):
                print("Correct answer. Score += 1")
                score += 1
                questions_asked += 1
    ...
    username = input(f"You reached a score of: {score}. Percentage score: {int((score/questions_asked)*100)}%. Enter a username: ")
    current_time = datetime.datetime.now()
    
    if os.path.isfile("High_Scores.txt") :
        data = openFile("High_Scores.txt")
        writeToFile("High_Scores.txt", [[str(current_time)[:10], username+" - "+str(score)]], "a+")
    else:
